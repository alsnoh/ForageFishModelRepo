...
pars <- c(1)#c(1:(nrow(CONSTANTS_original)-1)) 
#pars <- (1:1)
...
for (par in pars) {
  for (j in n) {
    ...
    # for finding trends between metabolism, asssimilation parameters and asymptotic length (Constant inputs and growth over many years)
    lastYear <- max(FoF$year)
    asymLength <- tail(FoF$Length[FoF$year == lastYear], 1) # asymptotic length
    assim <- tail(FoF$assimilation[FoF$year == lastYear], 1) # assimilation at end of year
    metaConst <- tail(FoF$metaConst[FoF$year == lastYear], 1) # metabolism constant at end of year

    asymInvestigation <- rbind(asymInvestigation, data.frame(param = par, delta = j, asymLength = asymLength, assim = assim, metaConst = metaConst))
  }
}
